...
**Generated:** 29 August 2026 · **Revised:** 29 August 2026 (documentation reconciliation pass: stale "planned/not yet built" framing removed across the build/design documents now that Windows AD Phase 1 and the Linux domain join are complete; the split DNS authority for `uow-csf.internal` recorded as a new blocking design decision; document paths corrected from the old `claude/` Project-store prefix to the repository's `docs/` layout)
**Source:** This git repository, which is now the authoritative store (44 files: this blueprint plus 43 documents under `docs/` — reconnaissance activities `r-01`–`r-18`, exploitation write-ups `e-01`–`e-17`, and the build/design/index set `w-01-windows-ad-baseline-design`, `ad-integration`, `linux-handover-checklist`, `secgen-audit`, `webapps-README`, `final-linux-vm-optimisation`, `services-README`, `suggestions`)

**Note on earlier references:** prior revisions of this blueprint were assembled from a hosted Claude Project and cited documents under a `claude/` path prefix, alongside an `agent.md` and a `pompts/` prompt set. Those files are no longer part of the working tree, and every document referenced below now lives directly under `docs/`.
...
- Deliver a **coherent vulnerable infrastructure**, not an unrelated collection of CVEs: remote unauthenticated vulnerabilities, weak/misconfigured network services, information disclosure, credential discovery/cracking/reuse, authenticated attack paths, local privilege escalation (sudo/SUID/cron/filesystem weaknesses), web application targets, DNS/email attack surface, and eventual Windows/AD integration.
...
Outstanding SCENARIO CHANGE items must be tracked and applied to the master VM before final student distribution.
...
| Attacker platform | University-provided Kali VM (also VMware-based) |
...
| Distribution format | Completed VMware VM directory, compressed as 7-Zip |
| IP addressing policy | DHCP on the Linux VM during the exploitation/testing phase (disposable VM addresses `192.168.144.130`–`.132`, see Section 4); the AD-integration/handover documentation records the Linux VM at static/final `192.168.144.100` as `cav-csf-linux`. The Windows DC (`cav-csf-windows` / `uow-csf-dc`) is a deliberate, documented exception at static `192.168.144.200`, since DNS/SRV/Kerberos require a stable address |
...
- **Windows Server 2019 Active Directory domain controller** (`cav-csf-windows` / `uow-csf-dc`) — **Phase 1 (AD DS/DNS baseline) built and validated**, not design-only. See Section 5.1 for the full current build state.
...
- **Linux VM ↔ AD integration**: the Linux VM has been **joined to `uow-csf.internal`** via `realmd`/`sssd`. See Section 5.3 for what's confirmed done versus what the integration-design document still leaves open.
...
- **`r-13` — CUPS discovery after IP change (192.168.144.131 → .132).** Following a VM re-clone, re-established the full baseline against the new address and used `cups-browsed` debug logging (`DebugLogging file stderr`) plus `tcpdump` to obtain definitive log-level evidence of the full unauthenticated queue-creation/callback sequence — stronger confirmation than the raw HTTP callback alone, and the direct motivation for using a real IPP server (rather than raw sockets) in the full exploitation chain. **All CUPS-related work after this point uses `192.168.144.132`.**

### 4.1a Reconnaissance of the five newly added services

All five recon activities below share the same target (`192.168.144.132`), attacker (`192.168.144.129`), and starting position (no prerequisite exploit, no credentials). Each precedes and directly motivates its `e-1x` exploitation counterpart in Section 4.2a.
...
- **`r-17` — DNS enumeration.** Confirmed `53/tcp`+`53/udp` new since baseline, running BIND9 9.18.49 authoritative for `uow-csf.internal`. A single unauthenticated `dig @... uow-csf.internal AXFR` (corroborated by the `dns-zone-transfer` NSE script) disclosed the complete zone in one query — nine records, including `dc01.uow-csf.internal → 192.168.144.200` (a forward-looking lead for the not-yet-built Windows AD VM at the time this was written) and two currently-unreachable hosts, `vpn-internal` (`.150`) and `backup-legacy` (`.151`), confirmed unreachable via a full-port `nmap` sweep of both addresses. Suggested teaching level 5.
...
| `e-16` | DNS zone transfer | No CVE — `allow-transfer { any; }` in `named.conf.local` | Unauthenticated → full internal zone disclosure (nine records, incl. forward-looking `dc01` DC lead) | Standalone; corroborates `e-12`/`e-13`'s shared-host naming; no shell |
...
This section reflects the actual current state, as recorded in the project's build/design/handover documents (`docs/w-01-windows-ad-baseline-design`, `docs/ad-integration`, `docs/linux-handover-checklist`, `docs/secgen-audit`, `docs/webapps-README`, `docs/final-linux-vm-optimisation`), correcting the earlier "planned/design-only" framing in prior sections. This is a separate, later work-stream from the reconnaissance/exploitation record in Section 4, which remains accurate to the disposable-VM testing it documents.

Those source documents have themselves now been reconciled against this section, so the contradiction that previously existed — this blueprint recording Phase 1 as built while `w-01` still opened with "nothing has been built" and `ad-integration` described the domain join as merely planned — no longer exists in either direction.

### 5.1 Windows AD domain controller — Phase 1 baseline: built and validated, not design-only

The design document `docs/w-01-windows-ad-baseline-design.md` was itself revised in-project; the earlier Server Core / `dc01` / `UOW-CSF` design (originally captured in this blueprint) is explicitly superseded and is background only. Note that the superseded `dc01` hostname still survives in the Linux BIND9 zone as an A record pointing at `192.168.144.200`, which is now a live inconsistency rather than a forward-looking breadcrumb — see Section 5.3.
...
**Phase 1 (AD DS/DNS baseline): passed and validated.** Confirmed via `dcdiag /v`, `nltest /dsgetdc`, SRV/DNS resolution, `Get-Service` (ADWS/DNS/Kdc/Netlogon/DFSR all running), `SYSVOL`/`NETLOGON` shares present. Memory checkpoints: Checkpoint A (no AD roles) 767 MB available; Checkpoint B (AD DS/DNS idle) 659 MB available (~1.36 GB used, under the 1.7 GB fallback-to-Server-Core threshold) — Desktop Experience held.

**OU/user/group scaffolding: built**, matching the design exactly — 5 OUs (`Users`, `Groups`, `Service Accounts`, `Computers` under `OU=UOW-CSF`), 7 users (`analyst`, `mpatel`, `jreed`, `skhan`, `helpdesk01`, `svc-web`, `backup.operator`), 5 groups (`Lab-Students`, `IT-Helpdesk`, `Web-Services`, `Backup-Operators-Lab`, `Staff-Admin` [unpopulated placeholder]), 4 populated memberships. **No Phase 2 vulnerable properties applied yet** (no SPNs, no `DoesNotRequirePreAuth`, no description-field content, no ACL changes) — this is still clean Phase 1 scaffolding, with naming chosen to anticipate where Phase 2 weaknesses will likely attach. Phase 1 baseline password convention: `CavLab2026!` (a deliberate build/testing convention, to be revised during Phase 2 vulnerability design, not an accidental weakness).

Power/lock-timeout settings applied permanently (`powercfg`/registry, based on prior 2008-server experience). Windows 7 and Windows 10 client domain join confirmed working end to end (tested account: `uowcsf\analyst`, DNS resolved correctly via domain join with no manual override).

**Still genuinely outstanding** (confirmed unchanged against the current `w-01`/`ad-integration` documents): master reproduction of this baseline at `192.168.144.200` is in progress (not yet complete); the DHCP decision for the lab segment (the DC and the tested Windows 10 client both currently require static IP — no DHCP available; the Windows 10 test client needed `192.168.144.237` configured statically); Linux-to-DC DNS/reachability checks (`cav-csf-linux` resolving/reaching `uow-csf-dc.uow-csf.internal`); whether the Linux VM should use the Windows DC as its primary DNS server versus only resolving/referencing it, and whether DNS fallback should remain available when the Windows VM is offline (both still open per `ad-integration`); which Linux services should carry AD breadcrumbs, and whether SMB integration should be Linux→Windows, Windows→Linux, or both (also `ad-integration`); the website component (Section 5.2 covers what's actually decided vs. open); Phase 2 vulnerability design entirely (Kerberoasting, AS-REP roasting, `DnsAdmins`/DCSync abuse remain design-proposed with worked-example commands recorded in `w-01` section 10, but not built or validated against a real account); snapshot-naming reconciliation with the Linux VM's convention; licensing (180-day eval vs. university channel); internet access during the build (needed for Windows Update, open decision on whether to remove it once done); and a shared NTP time source for Kali/the Linux VM once cross-machine Kerberos exercises are added, since clock skew can silently break exploitation attempts.

### 5.2 Web application training platforms: deployed, not "still to add"

Five platforms are live and DNS-mapped under `uow-csf.internal` (records alongside the infrastructure service records in `db.uow-csf.internal`) — this corrects the earlier "still to add: WebGoat, WebWolf, Security Shepherd" framing entirely, and adds a platform (Juice Shop) not previously recorded in this project's documentation at all:
...
The DNS infrastructure this table depends on (`uow-csf.internal`, `db.uow-csf.internal`) is therefore also confirmed live, correcting the earlier "still to add: DNS service" line.

The Windows-hosted website (Section 5.1's "website component") is a **separate, still-undecided** item: the *role* is agreed (a reconnaissance/initial-access bridge into the AD environment — staff names, department/group-matching naming, internal hostnames, breadcrumbs toward `uow-csf-dc.uow-csf.internal` — not a general OWASP-style playground duplicating the five platforms above), but the *implementation* is an open choice between a lightweight IIS/custom intranet (current lean, lowest resource cost), WordPress (IIS + PHP/FastCGI + tuned MariaDB, only justified if a specific plugin-CVE teaching objective needs it), or another lightweight option — to be settled once Phase 1 memory headroom is fully measured and the Phase 2 AD objects the site needs to reference actually exist.

### 5.3 Linux VM ↔ Windows AD integration

**Confirmed done** (per the handover checklist): the master Linux VM has been **successfully joined to `uow-csf.internal`** via `realmd`/`sssd`, using a dedicated `svc-linux-auth` service account; login and home-directory auto-creation both confirmed working.

**Blocking issue — split DNS authority for `uow-csf.internal`.** This was not previously recorded in any project document and is the single item most constraining the remaining integration work. The Linux VM runs BIND9 authoritative for `uow-csf.internal`, serving both the infrastructure service records and the five web-application records in `db.uow-csf.internal`; the Windows DC now holds an AD-integrated primary zone for the same name, auto-populated with the SRV records AD requires. Two servers therefore claim authority for one zone, and the consequences run in both directions: repointing the Linux resolver at the DC breaks hostname access to WebGoat, WebWolf, DVWA, Security Shepherd and Juice Shop unless those records are recreated in AD DNS, while leaving the Linux VM self-resolving means AD SRV lookups (`_ldap._tcp`, `_kerberos._tcp`) do not resolve from it, limiting any Kerberos-dependent exercise driven from the Linux side. Candidate resolutions — conditional forwarding of `_msdcs`/SRV lookups to the DC, replication of the application records into the AD-integrated zone, or a documented split-horizon design — are recorded in `docs/ad-integration.md`, none yet chosen. Any solution must preserve the deliberate `allow-transfer { any; }` misconfiguration on the Linux instance, since `e-16` depends on it entirely. The outstanding Linux-to-DC DNS/reachability checks (Section 5.1) cannot be meaningfully completed before this is settled.

A related, smaller item: the Linux zone advertises `dc01.uow-csf.internal → 192.168.144.200`, written as a forward-looking breadcrumb when no DC existed. The DC now exists at that address but answers to `uow-csf-dc`, so the address is right and the name is wrong. Either correct the record as a `SCENARIO CHANGE`, or retain `dc01` as a deliberately stale/legacy record — realistic in its own right, but it should be an explicit decision rather than an unnoticed drift.

**Still open**, per the integration-design document's own open-decisions list (the domain join answers only the first of these, which is now recorded there as resolved): whether the Linux VM should use the Windows DC as its primary DNS server (constrained by the blocking issue above); whether DNS fallback should remain available when the Windows VM is offline; which specific Linux services should carry breadcrumbs pointing toward AD; whether Kerberos/LDAP/SSSD-based exercises belong in the core lab or as an advanced extension; and whether any SMB integration should be Linux→Windows, Windows→Linux, or both.
...
**Not yet reviewed at all** (student-facing polish, distinct from the backend cleanup above): landing page wording/colours, login banner/MOTD content, the GitHub issue-reporting link, hostname/IP references across any visible documentation or on-VM content, references to the Windows DC hostname/domain, and any remaining wording implying the VM is still SecGen-generated (a SecGen acknowledgement belongs only in the GitHub README/docs, never the landing page or login banner).
...
A conservative, staged procedure is defined in `docs/final-linux-vm-optimisation.md`, to run only after Linux configuration, Windows AD integration, and headless conversion are all complete and validated. Linux configuration and headless conversion are done; AD integration is at the domain-join stage but has the unresolved DNS authority question in Section 5.3 sitting behind it, and Windows AD Phase 2 does not exist yet. Since the procedure's own regression testing explicitly covers Windows AD integration and cross-VM attack paths, running it now would validate against an incomplete target and need repeating — it should be sequenced after Phase 2. The procedure itself has **not yet been executed** — it remains a forward plan, not a completion record:
