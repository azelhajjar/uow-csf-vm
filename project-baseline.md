...
Initial exploitation and validation happens against a **disposable exploitation/test VM** first. Once a scenario change is understood and reproduced safely there, it is applied to the master teaching VM; final documentation records only the validated master state. Every disposable-VM change made during testing must be explicitly classified:
...
**No Windows Phase 1 build item remains open in this blueprint.** The remaining Windows/AD work is post-Phase-1 work: the lab segment remains static-addressed rather than DHCP-backed; the tested Windows 10 client needed `192.168.144.237` configured statically. The Windows-hosted website component is still undecided (Section 5.2 covers the role versus implementation choice); Phase 2 AD vulnerability design is now substantially built: Kerberoasting (`svc-web`, `e-18`), AS-REP roasting (`helpdesk01`, `e-19`), and DCSync domain replication rights abuse (`backup.operator`, `r-21`/`e-21`) are all built and validated on the master DC, with DCSync as the Windows Phase 2 capstone (full domain compromise, no further escalation to build); `DnsAdmins` group-membership abuse remains parked at the design/testing stage, reconnaissance is workable but no clean Kali-native student exploitation path has been validated, worked-example commands are still recorded in `w-01` section 10; snapshot-naming reconciliation with the Linux VM's convention remains open; licensing still needs final settlement (180-day eval versus university channel); and a shared NTP time source for Kali/the Linux VM should be decided before cross-machine Kerberos exercises are added, since clock skew can silently break exploitation attempts.
...
- **Windows/AD Phase 2**: Kerberoasting (`svc-web`, `e-18`), AS-REP roasting (`helpdesk01`, `e-19`), and DCSync domain replication rights abuse (`backup.operator`, `r-21`/`e-21`) are all built and validated, with DCSync as the Windows Phase 2 capstone. `DnsAdmins` group-membership abuse remains parked at the design/testing stage: reconnaissance is workable, but no clean Kali-native student exploitation path has been validated. AD CS remains a possible later Phase 3 direction, not part of the current built baseline.
