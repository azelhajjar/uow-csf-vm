...
**Generated:** 29 August 2026 · **Revised:** 31 August 2026, 1 September 2026 (project-state reconciliation pass: the Linux master is now recorded consistently as static `192.168.144.100` and already joined to the Windows AD domain; Windows Phase 1 is recorded as complete, with the Windows Server 2019 DC `uow-csf-dc` at `192.168.144.200` for `uow-csf.internal`; stale archive wording that still described `.130`, deferred static migration, incomplete landing-page review, or Phase 1 reproduction in progress has been superseded by the current verified state; split DNS is retained as a deliberate, tested, accepted design; all five web-application platforms are recorded as deployed; delivery of the Linux VM to the lab technician is treated as complete; 1 September update: Kerberoasting, AS-REP roasting, and DCSync domain replication rights abuse are now all built and validated Windows Phase 2 activities against the master DC, with DCSync via `backup.operator` (`r-21`/`e-21`) as the Windows Phase 2 capstone; `DnsAdmins` group-membership abuse has been dropped from the current lab: reconnaissance was workable, but no clean Kali-native student exploitation path could be validated, so the technique is not included in Phase 2; 2 September update: an IIS FTP service (`Microsoft ftpd`, TCP 21) is now built and validated on the master DC, offering anonymous read-only access to seven harmless IT Support/reference files, reconnaissance-only, see `r-23`; a WordPress/IIS service (WordPress 6.6.2, PHP 8.1.31 NTS via FastCGI, MariaDB 10.6.22 bound to `127.0.0.1` only) is now also built and validated on the master DC at `uow-news.uow-csf.internal`, see `w-01` section 15; a low-privilege domain-join account (`dcjoin`) is now built, delegated Create/Delete Computer object rights scoped to `OU=Computers,OU=UOW-CSF` only, with the domain's default computer container redirected there (`redircmp`) so standard client joins land in that OU without an explicit OU parameter, confirmed end to end with a Windows 10 test client join, see `w-01` section 3)
...
Accepted scenario changes are recorded only after they have been reproduced and validated on the master image. Documentation records the validated master state only: `cav-csf-linux` at `192.168.144.100`, `uow-csf-dc` at `192.168.144.200`, and domain `uow-csf.internal`. Temporary testing environments, failed attempts, and transient artefacts are not part of this repository's current-state documentation. Every accepted change is explicitly classified before being recorded:
...
- **EXPLOITATION ARTEFACT — do not replicate to master**: anything produced *as a consequence* of exploitation (attacker-added sudoers entries, dropped payloads/shells, attacker cron entries, changed group membership, persistence mechanisms).
...
| IP addressing policy | Final/master lab addressing is static: Linux master `cav-csf-linux` at `192.168.144.100`; Windows Server 2019 DC `cav-csf-windows` / `uow-csf-dc` at `192.168.144.200` for `uow-csf.internal`. A DHCP scope (`192.168.144.105`–`.125`) is also built on the DC for other clients, see `w-01` section 13 |
...
**Exploitation / credential attacks:** Metasploit Framework (module-driven exploitation and auxiliary scanners — `ftp_anonymous`, `ssh_version`, `mysql_version`/`mysql_login`/`mysql_authbypass_hashdump`, `http_version`, `apache_druid_js_rce`, `apache_druid_cve_2023_25194`, `distcc_exec`, `proftpd_133c_backdoor`, `ssh_erlangotp_rce`), John the Ripper (offline hash cracking against `rockyou.txt`), `impacket` (`GetUserSPNs`, `GetNPUsers` — exercised in `e-18`/`e-19`; `secretsdump` — exercised in `e-21`, DCSync via `backup.operator`), `bloodhound-python` (LDAP-based ACL/replication-rights collection, exercised in `r-21`), `nxc`/netexec (credential validation, exercised in `e-21`), `hashcat` (`-m 13100` Kerberoasting and `-m 18200` AS-REP — both exercised, see `e-18`/`e-19`), GTFOBins-style local privilege-escalation techniques, `journalctl`/`tcpdump` for target-side debug verification (CUPS chain).
...
- **`r-21` — DCSync replication rights enumeration** (file `r-21- dcsync-rights-enumeration.md`). Using the `analyst` credential from `r-19`, a `bloodhound-python` LDAP-based collection against the domain controller, parsed directly from its JSON output rather than a GUI, identifies `backup.operator` as holding the `DS-Replication-Get-Changes` and `DS-Replication-Get-Changes-All` extended rights on the domain object, a deliberate ACL misconfiguration with no corresponding administrative group membership. Feeds directly into `e-21`.
- **`r-23` — Windows service discovery: FTP** (file `r-23-windows-service-discovery.md`). Anonymous, unauthenticated reconnaissance of an IIS FTP service (`Microsoft ftpd`, TCP 21) on `uow-csf-dc`, exposing seven static IT Support reference files, password-reset contact guidance, department share paths, a new-starter checklist, and department contacts, without a credential. Reconnaissance-only, corroborating the naming convention already established in `r-19` and `r-22`.
...
Three Phase 2 AD credential attacks have been built and validated against the master Windows domain controller, `uow-csf-dc` (`192.168.144.200`), building on the reconnaissance in `r-19`/`r-21`. Kept as a separate table from the Linux VM's exploitation chain above (Section 4.2), since these are Windows/AD-specific, reached through Kerberos/LDAP, and not part of the Linux service surface.
...
| `e-21` | DCSync: domain replication rights abuse via `backup.operator` | No CVE — `backup.operator` granted `DS-Replication-Get-Changes`/`DS-Replication-Get-Changes-All` directly on the domain object (`r-21`) | Authenticated domain user (`backup.operator`) → full directory replication (DCSync): NTLM hashes and Kerberos keys for every domain account, including `krbtgt` | Windows Phase 2 capstone — full domain compromise, no further escalation to build |

`e-18` and `e-19`'s recovered credentials were independently confirmed via a freshly issued Kerberos TGT and an authenticated SMB session (`SYSVOL` read), not inferred from the offline crack alone; neither account carries elevated rights in the current build, so neither yields administrative access to the DC on its own. `e-21` is categorically different: `backup.operator`'s over-granted replication rights, confirmed as an ACL misconfiguration in `r-21` rather than a group-membership one, are sufficient by themselves for full domain compromise, distinct in kind from `e-18`/`e-19`'s standalone credential exposure and from the Linux VM's local-privilege-escalation chains in Section 4.2.
...
This section reflects the actual current state, using the project's build/design/handover documents (`docs/w-01-windows-ad-baseline-design`, `docs/ad-integration`, `docs/linux-handover-checklist`, `docs/secgen-audit`, `docs/webapps-README`, `docs/final-linux-vm-optimisation`) as evidence while correcting their stale points where later project work superseded them. This is a separate, later work-stream from the reconnaissance/exploitation record in Section 4, which remains accurate to the validated activity it documents.

Important reconciliation rule: the archived source notes still contain a few older status phrases, for example the Linux master still being on an earlier DHCP-assigned address, static migration still being deferred, landing-page review still being incomplete, and Windows Phase 1 reproduction still being in progress. This blueprint resolves those contradictions in favour of the current state supplied for this revision: Linux is final/static at `.100`, the AD join works, Windows Phase 1 is complete on `.200`, the landing page has been updated and verified, and the Linux VM has already been handed over.
...
**OU/user/group scaffolding: built**, matching the design exactly — 5 OUs (`Users`, `Groups`, `Service Accounts`, `Computers` under `OU=UOW-CSF`), 8 users (`analyst`, `mpatel`, `jreed`, `skhan`, `helpdesk01`, `svc-web`, `backup.operator`, `dcjoin`), 5 groups (`Lab-Students`, `IT-Helpdesk`, `Web-Services`, `Backup-Operators-Lab`, `Staff-Admin` [unpopulated placeholder]), 4 populated memberships. `dcjoin` is a low-privilege domain-join account (`OU=Users,OU=UOW-CSF`): delegated Create/Delete Computer object rights on `OU=Computers,OU=UOW-CSF` only, with the domain's default computer container redirected there (`redircmp`), confirmed end to end with a Windows 10 client join landing correctly in that OU. **Two Phase 2 vulnerable properties have been applied**: an SPN on `svc-web` (Kerberoastable, `e-18`) and `DoesNotRequirePreAuth` on `helpdesk01` (AS-REP roastable, `e-19`). No description-field content or ACL changes have been applied to any account — the remaining scaffolding is still clean Phase 1, with naming chosen to anticipate where further Phase 2 weaknesses will likely attach. Phase 1 baseline password convention: `CavLab2026!` (a deliberate build/testing convention, to be revised during Phase 2 vulnerability design, not an accidental weakness).
...
**No Windows Phase 1 build item remains open in this blueprint.** The remaining Windows/AD work is post-Phase-1 work: the lab segment now has a working DHCP scope on the DC alongside static addressing (`w-01` section 13); an IIS FTP service (`Microsoft ftpd`, TCP 21) is now built and validated on the DC, offering anonymous read-only access to seven harmless IT Support/reference files (`w-01` section 14, `r-23`); the tested Windows 10 client remains configured statically at `192.168.144.237`. The Windows-hosted intranet is built and validated (`r-22`; Section 5.2 covers the build); a Windows-hosted WordPress/IIS service is also built and validated at `uow-news.uow-csf.internal` (`w-01` section 15); Phase 2 AD vulnerability design is now substantially built: Kerberoasting (`svc-web`, `e-18`), AS-REP roasting (`helpdesk01`, `e-19`), and DCSync domain replication rights abuse (`backup.operator`, `r-21`/`e-21`) are all built and validated on the master DC, with DCSync as the Windows Phase 2 capstone (full domain compromise, no further escalation to build); `DnsAdmins` group-membership abuse has been dropped from Phase 2: reconnaissance was workable, but no clean Kali-native student exploitation path could be validated; the design/testing notes are retained in `w-01` section 10 for reference; snapshot-naming reconciliation with the Linux VM's convention remains open; licensing still needs final settlement (180-day eval versus university channel); and a shared NTP time source for Kali/the Linux VM should be decided before cross-machine Kerberos exercises are added, since clock skew can silently break exploitation attempts.
...
The Windows-hosted website (Section 5.1's "website component") is a **separate, built and validated post-Phase-1 item**: a lightweight static IIS intranet at `uow-intranet.uow-csf.internal`, serving as a reconnaissance/initial-access bridge into the AD environment (staff names, department/group-matching naming, internal hostnames, breadcrumbs toward `uow-csf-dc.uow-csf.internal` — not a general OWASP-style playground duplicating the five platforms above). Its application pool identity is tied to the existing `svc-web` account, giving that account's SPN (`e-18`) a genuine running service behind it. A second Windows-hosted web service has since been added alongside the intranet: WordPress 6.6.2 on IIS, at `uow-news.uow-csf.internal` (site `UOW-CSF-WP`, `C:\inetpub\wwwroot\wp-news`), running PHP 8.1.31 (NTS) via IIS FastCGI against MariaDB 10.6.22, which listens on `127.0.0.1` only. Its application pool (`UOW-CSF-WP`, default `ApplicationPoolIdentity`) and its database user (`wp_news_user`@`127.0.0.1`, database `wp_news`) are both local to this service, kept separate from `svc-web` and from any domain account. See `docs/w-01-windows-ad-baseline-design.md` section 15 for the full build record. See `docs/r-22-windows-intranet-reconnaissance.md` for the reconnaissance write-up.
...
Older non-static addresses referenced in the archive are build history; they no longer describe the master VM's current address, which is confirmed static at `192.168.144.100`.
...
- No earlier non-static IP references remaining under `/etc`, `/var/www`, `/opt`.
...
6. Full regression testing after each removal stage (boot, `eth0`, SSH, DNS, all listeners/services, all web apps, scheduled jobs, local privesc scenarios, deliberately-retained vulnerable package versions, Windows AD integration, cross-VM attack paths), always validated before being replicated to the master (Section 1.5).
...
- **Windows/AD Phase 2**: Kerberoasting (`svc-web`, `e-18`), AS-REP roasting (`helpdesk01`, `e-19`), and DCSync domain replication rights abuse (`backup.operator`, `r-21`/`e-21`) are all built and validated, with DCSync as the Windows Phase 2 capstone. `DnsAdmins` group-membership abuse has been dropped from Phase 2: reconnaissance was workable, but no clean Kali-native student exploitation path could be validated, so it is not included in the current lab. AD CS remains a possible later Phase 3 direction, not part of the current built baseline.
- **Windows-hosted website/intranet**: built and validated, a lightweight static IIS intranet at `uow-intranet.uow-csf.internal`, serving as an AD reconnaissance/initial-access bridge, tied to the existing `svc-web` account and SPN (`e-18`). See `docs/r-22-windows-intranet-reconnaissance.md`. `docs/r-23-windows-service-discovery.md` remains the FTP/service-discovery recon file.
- **Windows-hosted WordPress service**: built and validated, WordPress 6.6.2 on IIS at `uow-news.uow-csf.internal` (site `UOW-CSF-WP`, PHP 8.1.31 NTS via IIS FastCGI, MariaDB 10.6.22 bound to `127.0.0.1` only, database `wp_news`), with its own application pool identity and local database user, kept separate from the intranet's `svc-web`-tied app pool and from any domain account. The exploitation path is documented in `e-22` (WP File Manager arbitrary file upload); a dedicated reconnaissance write-up remains future work, pending `r-24`.
- **DHCP**: built and validated on the DC (`w-01` section 13); fixed lab infrastructure (`.100`, `.200`) and the static Windows 10 test client (`.237`) remain manually addressed, with a separate lease pool (`.105`–`.125`) available for other clients.
