...
| Purpose | Dedicated teaching domain, purpose-registered to support passive reconnaissance exercises and to connect the Linux VM (192.168.144.100) and the Windows/AD VM (192.168.144.200) into a single fictional organisation |
...
The Linux VM (192.168.144.100) is addressed purely by internal IP on an isolated lab segment. On its own, it has no domain, no public DNS presence, and nothing for passive OSINT techniques to act on, which is why passive reconnaissance was originally scoped as not applicable.
