...
```bash
nmap -sU -p 161 --script snmp-sysdescr,snmp-processes,snmp-netstat,snmp-interfaces --script-args snmpcommunity=public 192.168.144.100
```
...
```
[+] 192.168.144.100:161 - Login Successful: public (Access level: read-only); Proof (sysDescr.0): Linux cav-csf-linux 6.1.0-27-amd64 #1 SMP PREEMPT_DYNAMIC Debian 6.1.115-1 (2024-11-01) x86_64
```
...
```bash
snmpwalk -v2c -c public 192.168.144.100 1.3.6.1.2.1.25.1.6.0
```
...
```bash
snmpwalk -v2c -c public 192.168.144.100 1.3.6.1.4.1.8072.1.3.2
```
