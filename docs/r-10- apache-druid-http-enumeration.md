...
```bash
curl -v http://192.168.144.100:8081/ 2>&1 | head -30
curl -v http://192.168.144.100:8082/ 2>&1 | head -30
curl -v http://192.168.144.100:8083/ 2>&1 | head -30
curl -v http://192.168.144.100:8091/ 2>&1 | head -30
curl -v http://192.168.144.100:8888/ 2>&1 | head -30
```
...
|---|---|---|
| 8081 | `302 Found` | `http://192.168.144.100:8081/unified-console.html` |
| 8082 | `404 Not Found` | none |
| 8083 | `404 Not Found` | none |
| 8091 | `404 Not Found` | none |
| 8888 | `302 Found` | `http://192.168.144.100:8888/unified-console.html` |
...
```bash
nmap -p 8081,8082,8083,8091,8888 --script http-title,http-headers,http-methods 192.168.144.100
```
...
```
```
[+] 192.168.144.100:8888  ( 302-http://192.168.144.100:8888/unified-console.html )
```
...
```
[-] Exploit failed [unreachable]: OpenSSL::SSL::SSLError SSL_connect returned=1 errno=0 peeraddr=192.168.144.100:8081 state=SSLv3/TLS write client hello: wrong version number
[-] 192.168.144.100:8081 - Check failed: The state could not be determined.
```
...
```
[*] 192.168.144.100:8081 - Cannot reliably check exploitability. No LDAP search query was received.
```
